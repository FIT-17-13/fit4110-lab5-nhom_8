...
- [x] **Image tags:** bạn đã build image với tag `v0.1.0-nhom_8` và push lên registry (ghcr.io hoặc Docker Hub). Xác nhận rằng tag xuất hiện trong registry.

Ghi chú thêm những vấn đề gặp phải hoặc điều chỉnh tại đây:

```
- Cần chạy `docker network create class-net` trước lần compose đầu tiên.
- Newman test pass 16/16 assertions (reports/newman-lab05-compose.xml).
- Image local: fit4110/iot-ingestion:v0.1.0-nhom_8, fit4110/ai-service:v0.1.0-nhom_8.
```
